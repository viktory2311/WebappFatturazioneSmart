            ...
            fetch("/utenti/")
              ...
              .then(data => {
                                ...
                                originalData = data.map(u => ({
                                  ...
                                  "tariffa": u.tariffa,
                                  "Totale": u.totale_ore,
                                  "Totale_foglifirma": u.totale_foglifirma,
  ...
  try {    
    if (source === "oss") {
      ...
      const dataWithSource = data.map(row => ({
        ...row,
        descrizione: [row["Cognome BADATO"], row["Nome BADATO"]].filter(Boolean).join(" ") || row["Ragione sociale"] || "Nome non disponibile",        
        Fonte: source,
        apl: deriveAPL(source),
...
function populateTable(data) {
  ...
  data.forEach(row => {
    ...
    Totale_foglifirma= row["Totale_foglifirma"],
    Totale_residuo_ore= row["Totale_residuo_ore"],
...
async function exportExcel() {  
  ...
  let tipologiaValue = row.tipologia;
    ...
    {
      tipoUtenza = "anziani_autosufficienti";
    ...
    if (tipoFattura === tipoUtenza || tipoFattura === "Tutte le categorie" || tipoFattura === "Tutte_Categorie_viatesso" ) {
